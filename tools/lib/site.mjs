...
// Gallery page listing the bundled demos, deployed at /demo/.
export function renderDemoGallery(demos) {
  const cards = demos.map((d) => `
    <a class="dcard" href="./${d.slug}/">
      <div class="swatch" style="background:${d.swatch}"></div>
      <div class="dbody">
        <div class="dname">${d.name}</div>
        <p class="ddesc">${d.description}</p>
        <div class="dmeta">${d.traits.map((t) => `<span class="chip">${t}</span>`).join("")}</div>
      </div>
    </a>`).join("\n");
  return `<!doctype html>
<html lang="en">
...
<title>Design Intelligence — live demos</title>
<style>
  *,*::before,*::after{box-sizing:border-box}
  :root{--ground:#F7F8FB;--surface:#fff;--ink:#161A22;--muted:#565F70;--line:#E3E7EE;--accent:#985313;--accent-bg:#FBF1E3;--chip:#EDEFF3;
    --serif:"Iowan Old Style","Palatino Linotype",Palatino,Cambria,Georgia,serif;
    --sans:"Segoe UI",-apple-system,BlinkMacSystemFont,Arial,sans-serif;--mono:"Cascadia Code",Consolas,monospace}
  @media (prefers-color-scheme:dark){:root{--ground:#0F131B;--surface:#161B25;--ink:#E7EAF1;--muted:#98A2B4;--line:#252C3A;--accent:#E7A64E;--accent-bg:#241B0E;--chip:#1C222E}}
  ...
  h1{font-family:var(--serif);font-weight:600;font-size:clamp(1.7rem,4vw,2.4rem);margin:0}
  .sub{color:var(--muted);max-width:58ch;margin:12px 0 0}
  .grid{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin:26px 0 40px}
  .dcard{display:flex;flex-direction:column;background:var(--surface);border:1px solid var(--line);border-radius:12px;overflow:hidden;text-decoration:none;color:inherit}
  .dcard:hover{border-color:var(--accent)}
  .dcard:focus-visible{outline:2px solid var(--accent);outline-offset:3px}
  .swatch{height:110px}
  .dbody{padding:14px 16px 16px;display:flex;flex-direction:column;gap:8px}
  .dname{font-family:var(--serif);font-weight:600;font-size:1.15rem}
  .ddesc{margin:0;color:var(--muted);font-size:14px}
  .dmeta{display:flex;flex-wrap:wrap;gap:6px}
  .chip{font-family:var(--mono);font-size:10.5px;color:var(--muted);background:var(--chip);padding:2px 8px;border-radius:999px}
  .back{font-family:var(--mono);font-size:13px}
  a.back{color:var(--accent)}
  footer{border-top:1px solid var(--line);padding:20px 0 50px;color:var(--muted);font-size:13px}
...
<div class="wrap">
  <header>
    <p class="eyebrow">Design Intelligence · live demos</p>
    <h1>One system. Opposite answers.</h1>
    <p class="sub">Both pages were generated by the same pipeline — brief → recommend → tokens → build → review — and both score 100/100. The difference is the <em>context</em>, and the system reasons from it.</p>
    <p style="margin-top:14px"><a class="back" href="../">← Back to the skill catalog</a></p>
  </header>
  <div class="grid">
${cards}
  </div>
  <footer>
    <p>Open source · <a href="${REPO}">${REPO.replace("https://", "")}</a></p>
...
<div class="wrap">
  <header>
    ...
    <p style="margin-top:14px"><a class="demo" href="./demo/">▸ See live demos — the same system, opposite contexts (both scored 100/100)</a></p>
  </header>
